...
from algo.angel_module import GraphletAggregate, GraphletMessagePass, LogicCombineKG
...
class Angel(nn.Module):
    def __init__(
        ...
        self._edgetype_embedding = (th.rand((edgetype_num + 1, embed_size)) * 2 - 1) * math.sqrt(3)
        self._edgetype_embedding = nn.Embedding.from_pretrained(self._edgetype_embedding)  # .requires_grad_()
        ...
        self.NodeEmbeddingGenerate = nn.Sequential(
            nn.Linear(embed_dim + embed_size, embed_size), nn.ReLU(), nn.Linear(embed_size, embed_size)
        )
        # print("AAAA", self._istriangle)
        if self._istriangle:
        ...
        self.LogicCombine = LogicCombineKG(
            trianglelogic_num,
            squarelogic_num,
            combine_type,
            combine_embed,
    ...
    def forward(
        ...
        if self._istriangle:
            ...
            triangleneighborlogic = th.repeat_interleave(
                th.unsqueeze(trianglelogic, dim=2), self._triangleneighbornum, dim=2
            )
            ...
            for _logic in range(self._trianglelogicnum):
                ...
                for _node in range(3):
                    ...
                    _embed = self.NodeEmbeddingGenerate(
                        th.cat(
                            (
                                self._embedding(triangle_neighbor[:, _logic, :, _node]),
                                self._edgetype_embedding(triangleneighborlogic[:, _logic, :, _node]),
                            ),
                            dim=2,
                        )
                    )
                    ...
                    _embed = self.NodeEmbeddingGenerate(
                        th.cat(
                            (
                                self._embedding(triangle[:, _logic, :, _node]),
                                self._edgetype_embedding(triangleslogic[:, _logic, :, _node]),
                            ),
                            dim=2,
                        )
                    )
                    ...
                    _embed = self.NodeEmbeddingGenerate(
                        th.cat(
                            (
                                self._embedding(notriangle[:, _logic, :, _node]),
                                self._edgetype_embedding(triangleslogic[:, _logic, :, _node]),
                            ),
                            dim=2,
                        )
        ...
        if self._issquare:
            ...
            for _logic in range(self._squarelogicnum):
                ...
                for _node in range(4):
                    ...
                    _embed = self.NodeEmbeddingGenerate(
                        th.cat(
                            (
                                self._embedding(square_neighbor[:, _logic, :, _node]),
                                self._edgetype_embedding(squareneighborlogic[:, _logic, :, _node]),
                            ),
                            dim=2,
                        )
                    )
                    ...
                    _embed = self.NodeEmbeddingGenerate(
                        th.cat(
                            (
                                self._embedding(square[:, _logic, :, _node]),
                                self._edgetype_embedding(squareslogic[:, _logic, :, _node]),
                            ),
                            dim=2,
                        )
                    )
                    ...
                    _embed = self.NodeEmbeddingGenerate(
                        th.cat(
                            (
                                self._embedding(nosquare[:, _logic, :, _node]),
                                self._edgetype_embedding(squareslogic[:, _logic, :, _node]),
                            ),
                            dim=2,
                        )
            ...
            # source_embed: batch_size, embed_size
        ...
        # print(type(relation_type), relation_type.device)
        # self._edgetype_embedding(relation_type)  # DEBUG
        # for i, t in enumerate(
        #     (
        #         self._embedding(source_id),
        #         self._edgetype_embedding(relation_type),
        #         self._embedding(target_id),
        #         triangle_embed,
        ...
        # ):
        #     print(i, t)
        # print(self.training, relation_type)
