...
from algo.angel_module import GraphletAggregate, GraphletMessagePass, LogicCombineHG
...
class Angel(nn.Module):
    def __init__(
        ...
        self._nodetype_embedding = (th.rand((nodetype_num, embed_size)) * 2 - 1) * math.sqrt(3)
        self._nodetype_embedding = nn.Embedding.from_pretrained(self._nodetype_embedding).requires_grad_()
        ...
        self.LogicCombine = LogicCombineHG(
            trianglelogic_num,
            squarelogic_num,
            combine_type,
            combine_embed,
    ...
    def forward(
        ...
        if self._istriangle:
            ...
            # graphlet_logic: batch_size, graphletlogic_num, graphletneighbor_num, 3 for triangle (4 for square)
                    ...
                    _embed = self.NodeEmbeddingGenerate(
                        th.cat(
                            (
                                ...
                                self._nodetype_embedding(triangleneighborlogic[:, _logic, :, _node]),
                            ),
                            dim=2,
                        )
                    )
                    ...
                    _embed = self.NodeEmbeddingGenerate(
                        th.cat(
                            (
                                ...
                                self._nodetype_embedding(triangleslogic[:, _logic, :, _node]),
                            ),
                            dim=2,
                        )
                    )
                    ...
                    _embed = self.NodeEmbeddingGenerate(
                        th.cat(
                            (
                                ...
                                self._nodetype_embedding(squareneighborlogic[:, _logic, :, _node]),
                            ),
                            dim=2,
                        )
                    )
                    ...
                    _embed = self.NodeEmbeddingGenerate(
                        th.cat(
                            (
                                ...
                                self._nodetype_embedding(squareslogic[:, _logic, :, _node]),
                            ),
                            dim=2,
                        )
                    )
